...
\begin{columns}[c] % [c] — для вертикального центрирования содержимого
    ...
    \column{0.5\textwidth}
         \begin{figure}[!tbp]
             \includegraphics[width=0.4\textwidth]{../figures/head_mesh.jpg}  
             \caption{Разрез объема головы $\Omega$ }
        \end{figure}
  \end{columns}
\end{block}
...
\begin{block}{Идея}
    \begin{enumerate}
        \item Требуется добиться разряженного распределения зарядов. Обычные методы регуляризации не решают данную задачу.
...
\begin{block}{Недостаток}
    Требуется интегрирование по всему объему головы, точнось интегрированя напрямую зависит от сходимости метода решения PDE.
\end{block}
\end{frame}
...
\begin{block}{Данные}
Участники: 13 человек, возраст 20–30 лет. 8 — не равюотали с интерфейсом SMR-BCI.5 — опытные пользователи SMR-BCI. Все — без известных неврологических или медицинских заболеваний.
Люди представляли движение правой руки, ввижение ног. Каждое воображение длилось 5 секунд после появления визуального сигнала.
\end{block}
\begin{block}{Результат}
Точность восстановления на датчиках незначительно отличаются для разных методов. Они уже хорошо отображают потенциалы в пространство источников.
\end{block}
\begin{table}[ht]
\centering
\begin{tabular}{l c}
\hline
Метод & RMSE на сенсорах\\
\hline
InPDE & 0.025 \\
Loretta & 0.028 \\
\hline
\end{tabular}
\caption{Сравнение средней квадратичной ошибки восстановления потенциала на датчиках}
\end{table}
\end{frame}
...
\begin{block}{Модель}
Смоделирована шапочка EEG на овальной голове, дискретизация 50~Гц, 2 источника сигналов. Преимущество такого эксперимента по сравнению с реальными данными в том, что известны истинные источники сигналов, а также потенциалы в глубине головы.
\end{block}
\begin{block}{Результат}
...
\hline
Метод & RMSE на сенсорах & RMSE в глубине головы \\
\hline
InPDE & 0.005 & 0.02 \\
Loretta & 0.007 & 0.1 \\
\hline
\end{tabular}
\caption{Сравнение точности восстановления потенциала на датчиках и в глубине головы}
\end{table}
\end{frame}
...
\begin{frame}
\frametitle{Шумные данные — визуализация}
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{../figures/phi_on_sensors.png}
\caption{Предсказание на сильно зашумленных данных}
\end{figure}
\end{frame}
...
\begin{enumerate}
    \item Предложено решение обратной задачи ЭЭГ с помощью физико-информированного подхода с энергетической регуляризацией поля $\int_\Omega \phi\rho\,dV$ и TV-подобным структурным штрафом.
    \item Доказаны несмещённость и сходимость Монте Карло-аппроксимации энергетического регуляризатора, строгая выпуклость функционала и единственность глобального минимума.
    \item Показано отличие от классических методов: в отличие от чисто датасетовой MSE-оптимизации, предложенный подход вводит физически обоснованный энергетический штраф, что стабилизирует восстановление внутренних полей при сопоставимом качестве на сенсорах.
    \item Экспериментально подтверждено, что предложенный метод сохраняет качество на датчиках и улучшает восстановление в глубине головы, что особенно важно для оценки источников и побочных задач.
\end{enumerate}
\end{frame}
